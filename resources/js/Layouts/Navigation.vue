...
<template>
    ...
    <div :class="$page.props.showingMobileMenu ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'"
        ...
        <nav class="mt-10" x-data="{ isMultiLevelMenuOpen: false }">
            ...
            <nav-link :href="route('categories.index')" :active="route().current('categories.index')">
                <template #icon>
            ...
            <nav-link :href="route('logout')" :active="route().current('logout')" method="post" as="button" type="button">
                <template #icon>
